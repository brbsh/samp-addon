...
public:
	...
	amxAsyncHTTP(boost::asio::io_service& io_service, std::string host, std::string query) : io_s(io_service), resolr(io_service), sock(io_service), readyFlag(false)
	{
		response.clear();
		response_code = -1;
		...
		rqStream << "User-Agent: boost/1.49.0 (Linux i386; Linux i686) SAMP-Addon/2015.02.19\r\n";
		rqStream << "Host: " << host << "\r\n";
		rqStream << "Accept: */*\r\n";
		rqStream << "Connection: close\r\n\r\n";

		boost::asio::ip::tcp::resolver::query res_query(host, "http");
		resolr.async_resolve(res_query, boost::bind(&amxAsyncHTTP::resolveHandle, this, boost::asio::placeholders::error, boost::asio::placeholders::iterator));
		asyncOPThread = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&amxAsyncHTTP::httpThread, this)));
	}
	...
	void resolveHandle(const boost::system::error_code& error, boost::asio::ip::tcp::resolver::iterator endpoint_iter)
	{
		...
		sock.async_connect(endp, boost::bind(&amxAsyncHTTP::connectHandle, this, boost::asio::placeholders::error, ++endpoint_iter));
	}
	...
	void connectHandle(const boost::system::error_code& error, boost::asio::ip::tcp::resolver::iterator endpoint_iter)
	{
		if(!error)
		{
			boost::asio::async_write(sock, req, boost::bind(&amxAsyncHTTP::writeHandle, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
		}
	...
	void writeHandle(const boost::system::error_code& error, std::size_t bytes_tx)
	{
		if(!error)
		{
			boost::asio::async_read_until(sock, res, "\r\n", boost::bind(&amxAsyncHTTP::statusReadHandle, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
		}
		else
		{
			response.assign(error.message());
	...
	void statusReadHandle(const boost::system::error_code& error, std::size_t bytes_rx)
	{
		if(!error)
		{
			...
			boost::asio::async_read_until(sock, res, "\r\n\r\n", boost::bind(&amxAsyncHTTP::headerReadHandle, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
		}
		else
		{
			response.assign(error.message());
	...
	void headerReadHandle(const boost::system::error_code& error, std::size_t bytes_rx)
	{
		if(!error)
		{
			...
			boost::asio::async_read(sock, res, boost::asio::transfer_at_least(1), boost::bind(&amxAsyncHTTP::readHandle, this, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
		}
		else
		{
			response.assign(error.message());
